...
export const TargetInspector = ({ category }: TargetInspectorProps) => {
  ...
  const targetData =
    category.targets && category.targets.length > 0
      ? category.targets[0]
      : null;
  ...
  const calculateProgress = () => {
    ...
    return Math.min(
      (targetData.assignedSoFar / targetData.targetAmount) * 100,
      100,
    );
  };
    ...
    <section className="bg-white rounded-lg shadow-sm border border-gray-200 mt-4">
      ...
      >
        <h2 className="text-sm font-semibold text-gray-800 flex items-center gap-2">
          ...
          {targetData && (
            <span
              className={`w-4 h-4 rounded-full ${isComplete ? "bg-green-500" : "bg-yellow-500"}`}
            ></span>
          )}
          Target
        </h2>
          ...
          {!targetData && !isCreatingTarget ? (
            <div className="space-y-3">
              ...
              <p className="text-sm text-gray-600">
                When you create a target, we will let you know how much money to
                set aside to stay on track over time.
              </p>
              <button
            ...
            <div className="space-y-4">
              <div className="space-y-1">
                <h3 className="text-sm font-semibold text-gray-800">
                  Set Aside Another ${formatCurrency(targetData!.targetAmount)}{" "}
                  Each Month
                </h3>
              ...
              {/*  Impact message*/}
              <div
                className={`p-3 rounded-lg ${isComplete ? "bg-green-50" : "bg-yellow-50"}`}
              >
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-mono font-semibold">
                    ${formatCurrency(targetData!.assignedSoFar)}
                  </span>
                </div>
                <hr className="border-gray-200" />
                <div className="flex justify-between">
